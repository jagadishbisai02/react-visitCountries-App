...
import CountriesLists from '../CountriesList'
...
class Countries extends Component {
  ...
  render() {
    ...
    return (
      <HomeContainer>
        <CountryHeading>Countries</CountryHeading>
        <CountriesListContainer>
          {countriesList.map(each => (
            <CountriesLists
              countryDetails={each}
              key={each.id}
              onClickVisitedBtn={this.onClickVisitButton}
            />
          ))}
        </CountriesListContainer>
        <CountryHeading>Visited Countries</CountryHeading>
        <VisitedCountryList>
          {updatedList.length === 0 ? (
            ...
            updatedList.map(each => (
              <VisitedCountry
                ...
                key={each.id}
                onClickRemoveBtn={this.onClickRemoveButton}
              />
            ))
          )}
        </VisitedCountryList>
